...
├─ pdfs/                      # Static IPID and full policy wording attachments
├─ services/pdf/              # Runtime policy certificate generation
...
2. The route generates the policy-specific `Policy details and certificate.pdf` in memory and passes its Buffer to `sendPolicyEmail(customerEmail, policyData, dynamicPolicyCertificate)`. Email delivery remains non-blocking; failures are logged and do not roll back the created policy.
3. The email contains the generated policy-specific certificate plus **2 unchanged static PDFs** from `pdfs/`:
